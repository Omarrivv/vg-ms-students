package pe.edu.vallegrande.msvstudents.config;
...
public class MongoConfig {
    ...
    public CommandLineRunner initData(ReactiveMongoTemplate mongoTemplate) {
        return args -> {
            // Limpiar colecciones (bloqueante para asegurar orden)
            mongoTemplate.dropCollection(Student.class)
                .then(mongoTemplate.dropCollection(ClassroomStudent.class))
                .block();
            ...
            // Guardar estudiantes y luego matrículas
            mongoTemplate.insertAll(List.of(student1, student2, student3))
                .collectList()
